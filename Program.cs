...
// ============================================================
// POST /save-workout  ─ שמירת תוכנית אימון מעודכנת (אחרי עריכה ידנית)
// מקבל userId + מערך תוכניות, מוחק את הישנות ושומר את החדשות בדאטהבייס
// ============================================================
app.MapPost("/save-workout", async (int userId, List<WorkoutData> workouts, WorkoutDbContext db) =>
{
    Console.WriteLine($"[SAVE-WORKOUT] Request for userId: {userId}, {workouts?.Count ?? 0} workouts");
 
    // ולידציה: חייבת להיות לפחות תוכנית אחת
    if (workouts == null || workouts.Count == 0)
        return Results.Json(new { success = false, message = "No workouts provided" });
    ...
    try
    {
        ...
        // ── שמירת התוכניות המעודכנות, יום אחר יום ──
        for (int i = 0; i < workouts.Count; i++)
        {
            var workoutPlan = new WorkoutPlan
            {
                UserId = userId,
                Name = workouts[i].name ?? $"Day {i + 1}",
                DayNumber = i + 1
            };
            ...
            // שמירת התרגילים של היום
            if (workouts[i].excercises != null)
            {
                for (int j = 0; j < workouts[i].excercises.Count; j++)
                {
                    var ex = workouts[i].excercises[j];
        ...
        await db.SaveChangesAsync();
        Console.WriteLine($"[SAVE-WORKOUT] Saved successfully for userId: {userId}");
 
        return Results.Json(new { success = true, message = "Workout saved" });
    }
    catch (Exception ex)
    {
        Console.WriteLine($"[SAVE-WORKOUT] ERROR: {ex.Message}");
        return Results.Problem($"Error: {ex.Message}");
    }
});
